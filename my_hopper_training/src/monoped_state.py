...
import tf #transforms, not tensorflow
import numpy
import math
...
class MonopedState(object):
    ...
    def calculate_reward_orientation(self, weight=1.0):
        ...
        yaw_displacement = curren_orientation.z - self.desired_yaw
    ...
    def assign_bins(self, observation):
        ...
        :return:
        """
        state_discrete = numpy.zeros(len(self.list_of_observations))
        for i in range(len(self.list_of_observations)):
            state_discrete[i] = numpy.digitize(observation[i], self.bins[i])
    ...
    def create_bins(self):
        ...
        parts_we_disrcetize = self.discrete_division

        self.bins = numpy.zeros((number_of_observations, parts_we_disrcetize))
        for counter in range(number_of_observations):
            ...
            self.bins[counter] = numpy.linspace(min_value, max_value, parts_we_disrcetize)
